...
from nms_optimizer import Grid, calculate_module_bonus # type: ignore
...
class TestCalculateModuleBonus(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.grid.set_supercharge(0, 0, True)

    def test_supercharged_slot_supercharge_enabled(self):
        supercharged_slots = [(0, 0)]
        total_bonus = calculate_module_bonus(self.grid, 0, 0, supercharged_slots)
        self.assertAlmostEqual(total_bonus, 2.5)

    def test_supercharged_slot_supercharge_disabled(self):
        self.grid.set_supercharge(0, 0, False)
        supercharged_slots = [(0, 0)]
        total_bonus = calculate_module_bonus(self.grid, 0, 0, supercharged_slots)
