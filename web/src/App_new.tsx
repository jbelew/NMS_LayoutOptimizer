            ...
            {techTree &&
              Object.entries(techTree).map(([type, technologies]) => (
                <React.Fragment key={type}>
                  <h2 className="text-2xl" style={{ color: "var(--gray-12)" }}>{type.toUpperCase()}</h2>
...
const OptimizationButton: React.FC<{ label: string; onClick: () => void; loading: boolean; tech: string }> = ({ label, onClick, loading, tech }) => {
  const hasTech = useGridStore((state) => state.hasTechInGrid(tech)); // Access hasTechInGrid from the store
  const resetGridTech = useGridStore((state) => state.resetGridTech);

  return (
    <Flex gap="2" align="center" className="mt-2 mb-2">
      <IconButton onClick={onClick} disabled={loading} variant="soft">
        {hasTech ? <UpdateIcon /> : <DoubleArrowLeftIcon />} {/* Conditional Icon */}
      </IconButton>
      <IconButton onClick={() => resetGridTech(tech)} disabled={!hasTech || loading} variant="soft"> {/* Call resetGridTech */}
        <ResetIcon />
      </IconButton>
